...
let handle_dash_separated_blocks (compile_b : Cmarkit.Block.t -> _)
    (blocks, meta) =
  let div ((attrs, am), blocks) =
    let blocks =
      match blocks with
      | [ b ] -> compile_b b
      | blocks -> Ast.Block.Blocks (List.map compile_b blocks, Meta.none)
    in
  ...
  | None -> Ast.Block.Blocks (List.map compile_b blocks, meta)
  | Some n ->
...
let stage1 defs read_file md =
  ...
  let res =
    let attrs _ = compile_attrs in
    let iterator = { Iterators.Mapper.default with attrs } in
    iterator.block iterator res
  in
  (res, htbl_include)
...
let of_cmarkit ~read_file ~(fm : Frontmatter.t) (doc : Cmarkit.Doc.t) =
...
let rec to_cmarkit_i ?(for_image = false) (i : Ast.Inline.t) =
  let open Cmarkit.Inline in
  match i with
  | Strikethrough ((inline, attrs), meta) ->
      let inline = to_cmarkit_i inline in
      Ext_strikethrough ((Strikethrough.make inline, attrs), meta)
  | Math_span ((ms, attrs), meta) -> Ext_math_span ((ms, attrs), meta)
  | Attrs_span ({ content; attrs }, meta) ->
      let content = to_cmarkit_i content in
      Ext_attrs (Attributes_span.make content attrs, meta)
  | Autolink al -> Autolink al
  | Break br -> Break br
  | Code_span cs -> Code_span cs
  | Emphasis (({ delim; inline }, attrs), meta) ->
      let inline = to_cmarkit_i inline in
      Emphasis ((Emphasis.make ~delim inline, attrs), meta)
  | Inlines (ils, meta) ->
      let ils = List.map to_cmarkit_i ils in
      Inlines (ils, meta)
  | Link (({ text; reference }, attrs), meta) when for_image ->
      let text = to_cmarkit_i text in
      Image ((Link.make text reference, attrs), meta)
  | Link (({ text; reference }, attrs), meta) ->
      let text = to_cmarkit_i text in
      Link ((Link.make text reference, attrs), meta)
  | Raw_html html -> Raw_html html
  | Strong_emphasis (({ delim; inline }, attrs), meta) ->
      let inline = to_cmarkit_i inline in
      Strong_emphasis ((Emphasis.make ~delim inline, attrs), meta)
  | Text t -> Text t
  | Svg { origin; _ } | Video { origin; _ } | Image { origin; _ } ->
      to_cmarkit_i ~for_image:true (Link origin)
  | Audio _ | Pdf _ | Hand_drawn _ -> Inlines ([], Meta.none)

let rec to_cmarkit_b (b : Ast.Block.t) =
  let open Cmarkit.Block in
  match b with
  | Blank_line bl -> Blank_line bl
  | Block_quote (({ indent; block }, attrs), meta) ->
      let block = to_cmarkit_b block in
      Block_quote ((Block_quote.make ~indent block, attrs), meta)
  | Blocks (bs, meta) ->
      let bs = List.map to_cmarkit_b bs in
      Blocks (bs, meta)
  | Code_block cb -> Code_block cb
  | Heading (({ layout; level; inline }, attrs), meta) ->
      let inline = to_cmarkit_i inline in
      Heading ((Heading.make ~layout ~level inline, attrs), meta)
  | Html_block html -> Html_block html
  | Link_reference_definition lrd -> Link_reference_definition lrd
  | List (({ type'; tight; items }, attrs), meta) ->
      let items =
        List.map
          (fun ( {
            ...
            let block = to_cmarkit_b block in
            let li =
              List_item.make ~before_marker ~marker ~after_marker
                ?ext_task_marker block
            in
            (li, meta))
          items
      in
      List ((List'.make ~tight type' items, attrs), meta)
  | Paragraph (({ leading_indent; inline; trailing_blanks }, attrs), meta) ->
      let inline = to_cmarkit_i inline in
      Paragraph
        ((Paragraph.make ~leading_indent ~trailing_blanks inline, attrs), meta)
  | Thematic_break br -> Thematic_break br
  | Included ((b, _), _) -> to_cmarkit_b b
  | Div ((b, _), _) -> to_cmarkit_b b
  | Slide (({ content; title = Some (title, attrs) }, _), _) ->
      let title = Heading.make ~level:1 (to_cmarkit_i title) in
      let content = to_cmarkit_b content in
      Blocks ([ Heading ((title, attrs), Meta.none); content ], Meta.none)
  | Slide (({ content; title = None }, _), _) -> to_cmarkit_b content
  | Slip ((slip, _), _) -> to_cmarkit_b slip
  | SlipScript _ -> Block.Blocks ([], Meta.none)
  | Carousel ((c, _), meta) ->
      let bs = List.map to_cmarkit_b c in
      Blocks (bs, meta)
  | MermaidJS cb -> Code_block cb
  | Math_block mb -> Ext_math_block mb
  | Table (({ indent; col_count = _; rows }, attrs), meta) ->
      let rows =
        List.map
          (fun ((row, meta), s) ->
            let row =
              ...
              | `Header l ->
                  let l =
                    List.map
                      (fun (inline, layout) -> (to_cmarkit_i inline, layout))
                      l
                  in
                  `Header l
              | `Data l ->
          ...
          rows
      in
      Ext_table ((Table.make ~indent rows, attrs), meta)
  | Standalone_attributes attrs -> Ext_standalone_attributes attrs
  | Attribute_definition (({ indent; label; attrs }, attrs2), meta) ->
      Ext_attribute_definition
        ((Attribute_definition.make ~indent label attrs, attrs2), meta)
...
let to_cmarkit { Ast.doc; defs; _ } = Cmarkit.Doc.make ~defs (to_cmarkit_b doc)
