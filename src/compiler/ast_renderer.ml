...
type t = {
  init_context : context -> Ast.t -> unit;
  inline : inline;
  block : block;
  doc : doc;
}
...
and inline = context -> Ast.Inline.t -> bool
and block = context -> Ast.Block.t -> bool
and doc = context -> Ast.t -> bool
  ...
  let make renderer b =
    {
      ...
      doc_ =
        {
          doc = Ast.Block.Blocks ([], Ast.Meta.none);
          files = Fpath.Map.empty;
          defs = Cmarkit.Label.Map.empty;
        };
    }

  let buffer c = c.b
  let renderer c = c.renderer
  let get_doc (c : context) = c.doc_
  let get_defs (c : context) = c.doc_.defs
