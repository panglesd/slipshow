...
let pos_in_inline inline ~pos =
  ...
  let is_in_attrs =
    match attrs with None -> false | Some (_, (_, meta)) -> pos_in ~pos ~meta
  in
...
let enter_attrs acc pos attrs =
  let kv =
    ...
    List.find_map
      (function
        | k, Some ((_, meta) as v) when pos_in ~pos ~meta -> Some (Value (k, v))
        | ((_, meta) as k), v when pos_in ~pos ~meta -> Some (Key (k, v))
        | _ -> None)
      kv
  in
  let class_ =
    ...
    List.find_map
      (function
        | (_, meta) as v when pos_in ~pos ~meta -> Some (Class v) | _ -> None)
      classes
  in
  let id =
    match Cmarkit.Attributes.id attrs with
    | Some ((_, meta) as id) when pos_in ~pos ~meta -> Some (Id id)
    | _ -> None
  in
  let attribute =
...
let get_leave_attribute _pos (_attr : Cmarkit.Attributes.t) = None
...
(* and get_leave_block pos (block : Cmarkit.Block.t) = *)
(*   let meta = Slipshow.Ast.Utils.Block.meta block in *)
(*   let attr = Slipshow.Ast.Utils.Block.get_attribute block in *)
(*   let is_in_attr = *)
(*     match attr with *)
(*     | None -> None *)
(*     | Some (_, (attr, meta)) -> *)
(*         if pos_in ~pos ~meta then get_leave_attribute pos attr else None *)
(*   in *)
(*   let is_in_block = if pos_in ~pos ~meta then enter_block pos block else None in *)
(*   match (is_in_block, is_in_attr) with *)
(*   | None, e | e, None -> e *)
            ...
            if not @@ pos_in_textloc ~pos ~loc then None
              ...
              List.find_map
                (fun (t, ploc) ->
                  ...
                  if pos_in_textloc ~pos ~loc then Some t else None)
                targets)
          actions)
    action_plan
