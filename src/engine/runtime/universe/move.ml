open Fut.Syntax
open Window

open struct
  module Window = Brr.Window
  module G = Brr.G
  module El = Brr.El
  module Ev = Brr.Ev
  module Console = Brr.Console
end

let move mode window target ~duration =
  let old_coordinate = State.get_coord () in
  let+ () = move_pure mode window target ~duration in
  let undo () = move_pure Fast.slow window old_coordinate ~duration in
  ((), undo)

let move_relative ?(x = 0.) ?(y = 0.) ?(scale = 1.) mode window ~duration =
  let coord = State.get_coord () in
  let dest =
    {
      Coordinates.x = coord.x +. x;
      y = coord.y +. y;
      scale = coord.scale *. scale;
    }
  in
  move mode window dest ~duration

let move_relative_pure ?(x = 0.) ?(y = 0.) ?(scale = 1.) mode window ~duration =
  move_relative mode ~x ~y ~scale window ~duration |> Undoable.discard

let add_margin margin (c : Coordinates.element) =
  { c with width = c.width +. margin; height = c.height +. margin }

let focus ?(duration = 1.) ?(margin = 0.) mode window elems =
  let coords_e = List.map (Coord_computation.elem window) elems in
  let coords_e = List.map (add_margin margin) coords_e in
  let current = State.get_coord () in
  let coords_w = Coord_computation.Window.focus ~current coords_e in
  move mode window coords_w ~duration

let focus_pure ?margin mode window elem =
  focus ?margin mode window elem |> Undoable.discard

let enter ?(duration = 1.) ?(margin = 0.) mode window elem =
  let coords_e = Coord_computation.elem window elem in
  let coords_e = add_margin margin coords_e in
  let coords_w = Coord_computation.Window.enter coords_e in
  move mode window coords_w ~duration

let up ?(duration = 1.) ?(margin = 0.) mode window elem =
  let coords_e = Coord_computation.elem window elem in
  let coords_e = add_margin margin coords_e in
  let current = State.get_coord () in
  let coords_w = Coord_computation.Window.up ~current coords_e in
  move mode window coords_w ~duration

let down ?(duration = 1.) ?(margin = 0.) mode window elem =
  let coords_e = Coord_computation.elem window elem in
  let coords_e = add_margin margin coords_e in
  let current = State.get_coord () in
  let coords_w = Coord_computation.Window.down ~current coords_e in
  move mode window coords_w ~duration

let center ?(duration = 1.) ?(margin = 0.) mode window elem =
  let coords_e = Coord_computation.elem window elem in
  let coords_e = add_margin margin coords_e in
  let current = State.get_coord () in
  let coords_w = Coord_computation.Window.center ~current coords_e in
  move mode window coords_w ~duration

let scroll ?(duration = 1.) ?(margin = 0.) mode window elem =
  let coords_e = Coord_computation.elem window elem in
  let current = State.get_coord () in
  if
    coords_e.y -. (coords_e.height /. 2.)
    < current.y -. (Constants.height () /. 2. *. current.scale)
  then up mode window elem ~margin ~duration
  else if
    coords_e.y +. (coords_e.height /. 2.)
    > current.y +. (Constants.height () /. 2. *. current.scale)
  then down mode window elem ~margin ~duration
  else Undoable.return ()
