...
let do_serve ~port cond =
  ...
  let content =
    ref (Slipshow.delayed "# hello" |> Slipshow.delayed_to_string)
  in
  (* let k s = *)
    ...
    @@ Dream.router
         [
           ...
           Dream.get "/onchange" (fun _ ->
               let* new_content = Lwt_condition.wait cond in
               let () =
                 match new_content with
                 | Proto.Update s -> content := s
                 | _ -> ()
               in
               let to_send = Proto.to_string new_content in
               Dream.respond ~headers:[ ("Content-Type", "text/plain") ] to_send);
         ]
  in
  (* Lwt.both  *) dream
(* wac *)
