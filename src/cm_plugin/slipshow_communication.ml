...
let preview ?slipshow_js ?options ?read_file () =
  let id = ref 0 in
  ...
  fun ~ms state content ->
    ...
    let+ () = Fut.tick ~ms in
    if my_id = !id then
      Previewer.preview ?slipshow_js ?options ?read_file state content

let update_slipshow ?slipshow_js ?options ?read_file () =
  let preview = preview ?slipshow_js ?options ?read_file () in
  fun state view ->
    let open Editor in
    let content =
...
let slipshow_plugin ?slipshow_js ?options ?read_file ~errors_el preview_element
    =
  let open Editor in
  let update_slipshow = update_slipshow ?slipshow_js ?options ?read_file () in
