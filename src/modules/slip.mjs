...
export default function Slip(name, fullName, actionL, ng, options) {
  ...
  this.generateActionList = function () {
    ...
    this.queryAll("slip-slip[enter-at]").forEach((slip) => {
      newActionList[slip.getAttribute("enter-at")] = new Slip(
        slip,
        "",
        [],
        ng,
  ...
  this.getActionList = () => {
    ...
    for (let i = 0; i <= this.getMaxNext(); i++) {
      if (this.pauseSlipList[i] instanceof Slip) ret[i] = this.pauseSlipList[i];
      else if (
        typeof actionList[i] == "function" ||
        actionList[i] instanceof Slip
      )
        ret[i] = actionList[i];
      else ret[i] = () => {};
    }
  ...
  this.getMaxNext = () => {
    ...
    let sumArray = this.queryAll(
      "[pause], [step], [auto-enter], [immediate-enter]",
    ).map((elem) => {
  ...
  this.unpause = (pause) => {
    if (pause.hasAttribute("static-at-unpause")) {
      if (pause.getAttribute("static-at-unpause") == "") this.makeStatic(pause);
      else
        pause
          .getAttribute("static-at-unpause")
          .split(" ")
          .map((strID) => {
    ...
    if (pause.hasAttribute("unstatic-at-unpause")) {
      if (pause.getAttribute("unstatic-at-unpause") == "")
        ...
        pause
          .getAttribute("unstatic-at-unpause")
          .split(" ")
          .map((strID) => {
    ...
    if (pause.hasAttribute("down-at-unpause")) {
      ...
      else this.moveDownTo("#" + pause.getAttribute("down-at-unpause"), 1);
    }
    if (pause.hasAttribute("up-at-unpause")) {
      if (pause.getAttribute("up-at-unpause") == "") this.moveUpTo(pause, 1);
      else this.moveUpTo("#" + pause.getAttribute("up-at-unpause"), 1);
    }
    if (pause.hasAttribute("center-at-unpause")) {
      if (pause.getAttribute("center-at-unpause") == "")
        this.moveCenterTo(pause, 1);
      else this.moveCenterTo("#" + pause.getAttribute("center-at-unpause"), 1);
    }
    ...
    if (pause.hasAttribute("reveal-at-unpause")) {
      if (pause.getAttribute("reveal-at-unpause") == "") this.reveal(pause);
      else
        pause
          .getAttribute("reveal-at-unpause")
          .split(" ")
          .map((strID) => {
            this.reveal("#" + strID);
          });
    ...
    if (pause.hasAttribute("hide-at-unpause")) {
      if (pause.getAttribute("hide-at-unpause") == "") this.hide(pause);
      else
        pause
          .getAttribute("hide-at-unpause")
          .split(" ")
          .map((strID) => {
            this.hide("#" + strID);
          });
    ...
    if (pause.hasAttribute("figure-next-at-unpause")) {
      pause
        .getAttribute("figure-next-at-unpause")
        .split(" ")
        .map((figureID) => {
    ...
    if (pause.hasAttribute("figure-previous-at-unpause")) {
      pause
        .getAttribute("figure-previous-at-unpause")
        .split(" ")
        .map((figureID) => {
          this.query("#" + figureID).figureStep--;
        });
    ...
    if (pause.hasAttribute("focus-at-unpause")) {
      if (pause.getAttribute("focus-at-unpause") == "") this.focus(pause);
      else this.focus("#" + pause.getAttribute("focus-at-unpause"));
    }
    if (pause.hasAttribute("emph-at-unpause")) {
      if (pause.getAttribute("emph-at-unpause") == "") this.makeEmph(pause);
      else
        pause
          .getAttribute("emph-at-unpause")
          .split(" ")
          .map((strID) => {
            this.makeEmph("#" + strID);
          });
    ...
    if (pause.hasAttribute("unemph-at-unpause")) {
      if (pause.getAttribute("unemph-at-unpause") == "") this.makeUnEmph(pause);
      else
        pause
          .getAttribute("unemph-at-unpause")
          .split(" ")
          .map((strID) => {
    ...
    if (pause.hasAttribute("unfocus-at-unpause")) {
      if (pause.getAttribute("unfocus-at-unpause") == "") this.unfocus(pause);
      else this.unfocus("#" + pause.getAttribute("unfocus-at-unpause"));
    }
  };
  this.incrPause = () => {
    let pause = this.query(
      '[pause], [auto-enter]:not([auto-enter="0"]), [immediate-enter]:not([immediate-enter="0"]), [step]',
    );
    // let pause = this.query("[pause]");
    if (pause) {
  ...
  this.doAttributes = () => {
    ...
    this.queryAll("*[mk-unemphasize-at]").forEach((elem) => {
      let unemphAt = elem
        .getAttribute("mk-unemphasize-at")
        .split(" ")
        .map((str) => parseInt(str));
    ...
    this.queryAll("*[emphasize-at]").forEach((elem) => {
      let emphAt = elem
        .getAttribute("emphasize-at")
        .split(" ")
        .map((str) => parseInt(str));
      if (emphAt.includes(actionIndex)) elem.classList.add("emphasize");
      else elem.classList.remove("emphasize");
    });
    this.queryAll("*[chg-visib-at]").forEach((elem) => {
      let visibAt = elem
        .getAttribute("chg-visib-at")
        .split(" ")
        .map((str) => parseInt(str));
    ...
    this.queryAll("*[figure-previous-at]").forEach((elem) => {
      let toFigureNext = elem
        .getAttribute("figure-previous-at")
        .split(" ")
        .map((str) => parseInt(str));
      if (toFigureNext.includes(actionIndex)) elem.figureStep--;
    });
  ...
  this.refreshAll = () => {
    actionList
      .filter((elem) => elem instanceof Slip)
      .forEach((subslip) => {
        subslip.refreshAll();
      });
    this.pauseSlipList
  ...
  this.doRefresh = () => {
    ...
    replaceSubslips(
      clone,
      subSlipList,
      this.sketchpadCanvas,
      this.sketchpadCanvasHighlight,
      this.element,
    );
  ...
  this.reObserve = () => {
    ...
    if (
      slipScaleContainer &&
      slipScaleContainer.classList &&
      slipScaleContainer.classList.contains("slip-scale-container")
    )
  ...
  this.moveDownTo = (selector, delay, offset) => {
    setTimeout(() => {
      ...
      let coord = this.findSlipCoordinate();
      let d =
        ((elem.offsetTop + elem.offsetHeight) / 1080 - 1 + offset) *
        coord.scale;
      this.moveWindow(coord.x, coord.y + d, coord.scale, this.rotate, delay);
      // this.currentX = coord.x;
      // this.currentY = coord.y+d;
  ...
  this.moveCenterTo = (selector, delay, offset) => {
    setTimeout(() => {
      ...
      let coord = this.findSlipCoordinate();
      let d =
        ((elem.offsetTop + elem.offsetHeight / 2) / 1080 - 1 / 2 + offset) *
        coord.scale;
      this.moveWindow(coord.x, coord.y + d, coord.scale, this.rotate, delay);
      // this.currentX = coord.x;
      // this.currentY = coord.y+d;
  ...
  this.getLineWidth = () => {
    ...
    else if (["drawing"].includes(this.getTool())) return this.lineWidth;
  ...
  this.delay = isNaN(parseFloat(this.element.getAttribute("delay")))
    ? 0
    : parseFloat(this.element.getAttribute("delay"));
  // canvas for drawing
  var that = this;
  ...
  this.reloadCanvas = () => {
    ...
    setTimeout(function () {
      ...
      that.resizeObserver = new ResizeObserver((entries) => {
        if (
          that.element.firstChild.offsetHeight > 0 &&
          that.element.firstChild.offsetWidth > 0
        ) {
  ...
  setTimeout(function () {
    ...
    if (element && element.firstChild && element.firstChild.firstChild)
      element.firstChild.firstChild.appendChild(canvas2);
    else element.appendChild(canvas2);
    setTimeout(function () {
      that.reloadCanvas();
    }, 0);
  }, 0);
  ...
  this.generatePauseFlowSlipList = function () {
    ...
    bla.forEach((elem) => {
      if (elem.hasAttribute("auto-enter")) {
        slipList[step] = new Slip(
          elem,
          elem.getAttribute("toc-title") || "",
          [],
          ng,
          {},
        );
      ...
      if (elem.hasAttribute("step")) {
        if (elem.hasAttribute("enter-at-unpause")) {
          if (elem.getAttribute("enter-at-unpause") != "") {
            ...
            slipList[step + (parseInt(elem.getAttribute("step")) || 1) - 1] =
              new Slip(elem, elem.getAttribute("toc-title") || "", [], ng, {});
        }
      ...
      if (elem.hasAttribute("pause")) {
        if (elem.hasAttribute("enter-at-unpause")) {
          if (elem.getAttribute("enter-at-unpause") != "") {
            let s = this.query(elem.getAttribute("enter-at-unpause"));
            slipList[step + (parseInt(elem.getAttribute("step")) || 1) - 1] =
              new Slip(s, s.getAttribute("toc-title") || "", [], ng, {});
          } else
            slipList[step + (parseInt(elem.getAttribute("step")) || 1) - 1] =
              new Slip(elem, elem.getAttribute("toc-title") || "", [], ng, {});
        }
