...
let do_watch input f =
  ...
  let callback prefix filename =
    if
      Fpath.Set.mem
        (Fpath.normalize @@ Fpath.( / ) prefix filename)
        !state_depending_on_files
    then (
  ...
  in
  let listened_directories = Fpath.Map.singleton parent (unwatch parent) in
      ...
      let new_listened_directories =
        Fpath.Set.fold
          (fun file map ->
            ...
            match Fpath.Map.find_opt parent listened_directories with None -> _ | Some 
            _)
