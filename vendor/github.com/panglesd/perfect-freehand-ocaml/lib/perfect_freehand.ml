...
module Point = struct
  ...
  let get_y v = Jv.Jarray.get v 1 |> Jv.to_float
...
module Options = struct
  ...
  let size t = Jv.get t "size" |> Jv.to_option Jv.to_float
  let thinning t = Jv.get t "thinning" |> Jv.to_option Jv.to_float
  let smoothing t = Jv.get t "smoothing" |> Jv.to_option Jv.to_float
  let streamline t = Jv.get t "streamline" |> Jv.to_option Jv.to_float
  let last t = Jv.get t "last" |> Jv.to_option Jv.to_bool
...
let get_stroke ?options x =
  ...
  let res =
    match options with
    | None -> Jv.apply get_stroke [| x |]
    | Some options -> Jv.apply get_stroke [| x; options |]
  in
