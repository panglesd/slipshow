var Slipshow = (function (exports) {
	...
	class Atrament extends AtramentEventTarget {
	  constructor(selector, config = {}) {
	    ...
	    const mouseMove = event => {
	      ...
	      if (mouse.down && PathDrawingModes.includes(this.modeInternal)) {
	        ...
	        if (!this.dirty && this.modeInternal === DrawingMode.DRAW && (x !== mouse.x || y !== mouse.y)) {
	          this.dirty = true;
	          this.fireDirty();
	        }
	        mouse.set(x, y);
	    ...
	    ['weight', 'smoothing', 'adaptiveStroke', 'mode'].forEach(key => {
	  ...
	  draw(x, y, prevX, prevY) {
	    ...
	    const smoothingFactor = Math.min(minSmoothingFactor, this.smoothing + (rawDist - 60) / 3000);
	    ...
	    if (this.adaptiveStroke) {
	      ...
	      this.targetThickness = (dist - minLineThickness) / lineThicknessRange * (this.maxWeight - this.weightInternal) + this.weightInternal;
	  ...
	  fill() {
	    ...
	    if (!this.filling) {
	      ...
	      this.fillStack.push([mouse.x, mouse.y, startColor]);
	    }
	  }
	...
	function Slip$1(name, fullName, actionL, ng, options) {
	  ...
	  this.unpause = pause => {
	    ...
	    if (pause.hasAttribute("emph-at-unpause")) {
	      if (pause.getAttribute("emph-at-unpause") == "") this.makeEmph(pause);else pause.getAttribute("emph-at-unpause").split(" ").map(strID => {
	        this.makeEmph("#" + strID);
	      });
	    }
	    if (pause.hasAttribute("unemph-at-unpause")) {
	      if (pause.getAttribute("unemph-at-unpause") == "") this.makeUnEmph(pause);else pause.getAttribute("unemph-at-unpause").split(" ").map(strID => {
	  ...
	  this.next = function () {
	    ...
	    if (this.pauseSlipList[actionIndex] instanceof Slip$1) return this.pauseSlipList[actionIndex];
	  ...
	  // element
	  this.element = typeof name == "string" ? document.querySelector(name[0] == "#" ? name : "#" + name) : name;
	  // scale, rotate, delay
	    ...
	    bla.forEach(elem => {
	      ...
	      if (elem.hasAttribute("step")) {
	        if (elem.hasAttribute("enter-at-unpause")) {
	          ...
	          } else slipList[step + (parseInt(elem.getAttribute("step")) || 1) - 1] = new Slip$1(elem, elem.getAttribute("toc-title") || "", [], ng, {});
	        }
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" focusable="false" width="20" height="20" style="-ms-transform: rotate(360deg); -webkit-transform: rotate(360deg); transform: rotate(360deg);" preserveAspectRatio="xMidYMid meet" viewBox="0 0 36 36"><path class="clr-i-outline clr-i-outline-path-1" d="M14.58 32.31a1 1 0 0 1-.94-.65L4 5.65a1 1 0 0 1 1.25-1.28l26 9.68a1 1 0 0 1-.05 1.89l-8.36 2.57l8.3 8.3a1 1 0 0 1 0 1.41l-3.26 3.26a1 1 0 0 1-.71.29a1 1 0 0 1-.71-.29l-8.33-8.33l-2.6 8.45a1 1 0 0 1-.93.71zm3.09-12a1 1 0 0 1 .71.29l8.79 8.79L29 27.51l-8.76-8.76a1 1 0 0 1 .41-1.66l7.13-2.2L6.6 7l7.89 21.2l2.22-7.2a1 1 0 0 1 .71-.68z" fill="#000000"/><rect x="0" y="0" width="36" height="36" fill="rgba(0, 0, 0, 0)" /></svg>
	  ...
	  this.updateCounter = function () {
	    ...
	    if (window.parent !== window) {
	      if (this.send_stage) window.parent.postMessage({
	        id: this.id,
	        kind: "state",
	        data: hash
	      }, "*");
	  ...
	  this.getSlipTree = function (slip) {
	    slip = slip || rootSlip;
	    if (slip instanceof Slip$1) return {
	      name: slip.name,
	      slip: slip,
	...
	let startSlipshow = async (startingPoint, id) => {
	  ...
	  });else engine = new Engine(document.querySelector("slip-slipshow")).start(startingPoint, id);
	  return Promise.resolve(engine);
	};
